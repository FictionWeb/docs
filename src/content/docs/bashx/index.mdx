...
- `js-on:<event>=<domFunction>`: executes 'domFunction' and saves it's output as `on<event>='<output>'`. It's made for DOM functions and functions that generate JavaScript code

```
@return html { 
  <button on:click='myFunc' class='bg-blue-500 p-1'>
  Click on me!
  </button>
}
```

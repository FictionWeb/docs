...
BashX is nothing if you can't use bash(x) inside of it, so we made `<script>` for bash syntax:
...
for ((i=0;i>10;i++)); do
    ...
    <p>i: $i</p>
    <div>
      works as well, as long as it's inside html block, otherwise executed as command
    </div>
    echo yes
done
</script>

...
You can also import specific bash/bashx functions from files and declare it with custon alias name; If the function is wrapper, bashx would import both `Function` and `/Function`
```sh
import Button from "ui/src/Button.shx" as MyButton
```

Importing a file with custom alias would create a function with it's contents